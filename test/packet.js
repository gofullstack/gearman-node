var assert = require("assert"),
    vows = require("vows"),
    packetTypes = require("../lib/packet/types"),
    packet = require("../lib/packet");

vows.describe("packet").addBatch({
    "types": {
        "names are loaded": function (topic) {
            assert.equal(packetTypes.names.SUBMIT_JOB, 7);
        },

        "numbers are loaded": function (topic) {
            assert.equal(packetTypes.numbers[7], "SUBMIT_JOB");
        }
    },

    "encode": {
        "is a function": function () {
            assert.ok(typeof packet.encode === "function");
        },

        "returns a Buffer": function () {
            assert.ok(packet.encode({ type: "SUBMIT_JOB",
                                      name: "test" }) instanceof Buffer);
        },

        "must have a known type": function () {
            assert.throws(function () { packet.encode({ type: "AAA",
                                                        name: "test" }); });
        },
        ...
        "SUBMIT_JOB": {
            topic: {
                ...
                empty: new Buffer([0,0x52,0x45,0x51,0,0,0,0x7,0,0,0,0x6,0x74,0x65,0x73,0x74,0,0]),
